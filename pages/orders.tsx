...
export default function Example() {
    ...
    let getOrders = async () => {
        ...
        const orderData =  Promise.all((await posts).map(order => Promise.all(order.map(post => post.data()))))

        // Format data into JSON, add order totals and status
        const formatedOrderData = (await orderData).map((order, index) => (
            {
                data: order,
                orderTotal: order.map(post => parseFloat(post.price)).reduce((a, b) => a + b, 0),
            }
        ))
